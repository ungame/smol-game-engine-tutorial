...
namespace smol 
{
    ...
    bool Platform::initOpenGL(int glVersionMajor, int glVersionMinor, int colorBits, int depthBits)
    {
        ...
        if(! pixelFormat )
        {
            ...
            WriteLogError("Nao foi possivel alocar um pixel format", __FILE__, __LINE__);
        ...
        if(! SetPixelFormat(dummyWindow->dc, pixelFormat, &pfd) ) 
        {
            ...
            WriteLogError("Nao foi possivel setar o pixel format", __FILE__, __LINE__);
            destroyWindow(dummyWindow);
            return false;
        }
        ...
        if (! rc )
        {
            ...
            WriteLogError("Nao foi possivel criar um contexto OpenGL valido", __FILE__, __LINE__);
            destroyWindow(dummyWindow);
            return false;
        }
        ...
        if (! wglMakeCurrent(dummyWindow->dc, rc) ) 
        {
            ...
            WriteLogError("Nao foi possivel setar o context OpenGL atual", __FILE__, __LINE__);
            destroyWindow(dummyWindow);
            return false;  
        }
    ...
    Window* Platform::createWindow(int width, int height, const char* title)
    {
        ...
        if(! GetClassInfoExA(hInstance, smolWindowClass, &wc) )
        {
            ...
            {
                LogError("Nao foi possivel registrar a classe da janela");
                WriteLogError("Nao foi possivel registrar a classe da janela", __FILE__, __LINE__);
                return nullptr;
            }
        }
        ...
        if (! windowHandle )
        {
            ...
            WriteLogError("Nao foi possivel criar uma janela", __FILE__, __LINE__);
            return nullptr;
        }
        ...
        if(globalRenderApiInfo.name == RenderAPIName::OPENGL) {
            ...
            if( numPixelFormats <= 0 )
            {
                ...
                WriteLogError("Nao foi possivel pegar um pixel format valido", __FILE__, __LINE__);
                return nullptr;          
            }
            ...
            {
                LogError("Não foi possivel setar o contexto OpenGL atual");
                WriteLogError("Não foi possivel setar o contexto OpenGL atual", __FILE__, __LINE__);
                return nullptr;
            }
