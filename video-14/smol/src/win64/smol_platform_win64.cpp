...
namespace smol 
{
    ...
    Module* Platform::loadModule(const char* path) {
        ...
        LogError("Falhou ao carregar modulo: %s", path);
        return nullptr;
    }
    ...
    bool Platform::unloadModule(Module* module) {
        ...
        {
            LogError("Erro ao descarregar modulo");
            return false;
        }
        delete module;
        return true;
    ...
    void* Platform::getFunctionFromModule(Module* module, const char* function)
    {
        ...
        if(! addr )
        {
            LogError("Falhou ao pegar o ponteiro para a funcao %s do module", function);
            return nullptr;
        }
    ...
    char* Platform::loadFileToBuffer(const char* fileName, size_t* loadedFileSize, size_t extraBytes, size_t offset)
    {
        ...
        {
            LogError("Falhou ao ler do arquivo '%s'", fileName);            
            delete[] buffer;
            buffer = nullptr;
        }
